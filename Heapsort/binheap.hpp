...
struct binheap {
    ...
    std::vector<T, Allocator> A;  /**< およそ完全2分木とみなせるone-based配列A */
    Compare cmp;                  /**< 比較用オブジェクト */

    explicit binheap(std::size_t n) : size(0), length(n), A(n + 1) {}
    template <class Iterator>
    binheap(Iterator a0, std::size_t n) : size(0), length(n), A(n + 1)
    {
        std::copy(a0, a0 + n, (A.begin() + 1));  // A[1..n]にコピー
    }
    template <class Iterator>
    binheap(Iterator a0, Iterator aN) : size(0)
    {
        ...
        A.resize(length + 1);                    // Aの記憶領域確保
        std::copy(a0, aN, (A.begin() + 1));      // A[1..n]にコピー
    }
    explicit binheap(const std::vector<T>&& v) : size(0), length(v.size())
    {
        A.resize(length + 1);                            // Aの記憶領域確保
        std::move(v.begin(), v.end(), (A.begin() + 1));  // A[1..n]にムーブ
    }
     ...
     *        配列A[1..n]をmax(min)ヒープに変換できる.部分配列A[(floor(n/2)+1)..n]の
    ...
    void build()
    {
        ...
        // 各節点i+1,i+2,..nはあるmax(min)ヒープの根である
        for (std::size_t i = length / 2;
             i > 0;      // i = 0で手続き終了
             --i) {      // iを1減らすことで次の繰り返しに対するループ不変式が再び成立する
            heapify(i);  // 節点iをmax(min)ヒープの根に修正する
            // また、このheapify呼び出しは節点i+1,i+2,..,nがmax(min)ヒープの根であるという性質を保存する
        }
        // 手続きはi = 0で終了する.ループ不変式から、各節点1,2,..nはmax(min)ヒープの根である
        // 特に節点1はmax(min)ヒープの根である
    }
     ...
     *
     *        最初に、buildを用いて入力配列A[1..n]上でmax(min)ヒープを構築する
     *        配列の最大(最小)要素は根A[1]に格納されているので、A[n]と交換することで
     *        最大(最小)要素を正しい最終位置に置く.次に、A.heap-sizeを1だけ減らして
     *        ヒープから節点nを削除する.このとき、根の左右の子はともにmax(min)ヒープを形成するが、
     *        更新された根はmax(min)ヒープ条件に違反する可能性がある.そこで、heapify(1)を
     *        呼び出してmax(min)ヒープ条件を修復すると、A[1..(n-1)]上にmax(min)ヒープが構築される
    ...
    void sort()
    {
        // 最初に、buildを用いて入力配列A[1..n]上でmax(min)ヒープを構築する
        build();
        // for文の各繰り返しの直前では、部分配列A[1..i]はA[1..n]の小さい(大きい)ほうから
        // i個の要素を含むmax(min)ヒープであり、部分配列A[i+1..n]はA[1..n]の大きい(小さい)ほうから
        // n - i個の要素をソートされた順序で含む
    ...
    {
        return A[i + 1];  // zero-basedからone-basedへ写像
    }
     ...
     */
    void update(std::size_t i, T key, bool check = true)
    {   
    ...
    void insert(T key)
    {
        ...
        update(size, key, false);  // A[size]のキー値を変更する
    }
    ...
    {
        assert(size > 0);  // ヒープアンダーフローチェック
        i = i + 1;         // zero-basedからone-basedへ写像
        A[i] = A[size];    // A[i]にA[size]を代入
